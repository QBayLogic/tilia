...
import System.FilePath (equalFilePath, normalise, splitDirectories, takeDirectory, (<.>), (</>))
...
data Component = Component
  { -- | Its source directories, absolute and canonical.
    ...
    -- | Files explicitly named by this component. Directory overlap alone
    -- does not make a unit test a module of the doctest driver beside it.
...
claiming file components =
  ...
  where
    ...
    named c = any (equalFilePath file) (componentFiles c)
  ...
  pure
    Component
      { componentDirs = concat dirs,
        componentFiles = [normalise (d </> f) | d <- concat dirs, f <- files],
...
buildInfos described =
  ...
  where
    ...
    executable e = with (buildInfo e) [modulePath e]
