...
-- declare.
--
-- "Linden.Fixity" resolves a module's operators exactly, given a function
...
-- == Finding the module
--
-- Which package exposes a module is answered by "Linden.Fixity.Cabal",
...
import Linden.Fixity
import Linden.Fixity.Cabal (packageModules)
import Linden.Fixity.Builtin (builtinFixities)
import Linden.Fixity.Cache
import Linden.Fixity.PackageDb
import Linden.Parser
...
-- | Build a lookup function for "Linden.Fixity".
--
-- 'Nothing' means the module could not be read, which is not the same as
-- its having no operators; see 'Linden.Fixity.resolveScope' for why the
-- difference has to survive.
--
-- Answers are remembered on disk between runs by "Linden.Fixity.Cache", so
...
--
-- This is the join between the pure half of "Linden.Fixity" and the half
...
-- 'Linden.Fixity.lookupFixity' distinguish a conclusion from a guess.
scopeFor ::
